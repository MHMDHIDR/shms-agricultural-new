 ...
 */
export function extractS3FileName(url: string, skipBucketName = true): string | null {
  try {
    const parsedUrl = new URL(url)
    // Remove the leading slash from pathname to get all path parts without skipping the bucket name
    const pathParts = parsedUrl.pathname.slice(1).split("/")
    // Join all parts after the bucket name to get the file key
    const fileKey = pathParts.slice(skipBucketName ? 0 : 1).join("/")
